...
export const GraphZ: React.FC<GraphZProps> = ({ curves, points }) => {
  const data = useMemo(() => {
    ...
    // Añadir puntos
    const validPoints = points.filter(p => p.calculatedValue !== null);
    if (validPoints.length > 0) {
      traces.push({
        x: validPoints.map(p => p.calculatedValue!.x),
        y: validPoints.map(p => p.calculatedValue!.y),
        type: 'scatter',
  ...
  const layout: Partial<Plotly.Layout> = useMemo(() => ({
    ...
    xaxis: {
      title: { text: 'x', font: { size: 14, color: '#a0a0b8' } },
      gridcolor: 'rgba(42, 42, 69, 0.8)',
      zerolinecolor: 'rgba(0, 245, 212, 0.3)',
      zerolinewidth: 1,
    ...
    yaxis: {
      title: { text: 'y', font: { size: 14, color: '#a0a0b8' } },
      gridcolor: 'rgba(42, 42, 69, 0.8)',
      zerolinecolor: 'rgba(0, 245, 212, 0.3)',
      zerolinewidth: 1,
      tickfont: { family: 'JetBrains Mono', size: 10 },
  ...
  return (
    <div className="graph-container">
      <div className="graph-title">Plano Z (x, y)</div>
      <Plot
        data={data}
        layout={layout}
        config={config}
